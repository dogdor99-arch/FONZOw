...
export default function ArtistDetail() {
  ...
  return <>
    <section className="border-b border-border/70 bg-cream/40 px-4 py-3 sm:px-6 lg:px-10"><div className="mx-auto max-w-[1400px] lg:pl-12"><Link href="/artists" className="inline-flex items-center text-[10px] uppercase tracking-[0.16em] text-muted-foreground hover:text-brand"><ArrowLeft className="mr-1.5 h-3.5 w-3.5" />{t("กลับไปศิลปินของ Fonzo", "Back to Fonzo artists")}</Link><h1 className="mt-2 font-display text-3xl leading-none sm:text-4xl">{title}</h1>{role && !["ศิลปินที่ Fonzo สนับสนุน", "Artists supported by Fonzo"].includes(role.trim()) && <p className="mt-2 text-[10px] tracking-[0.16em] text-brand uppercase">{role}</p>}{artist.guitar && <div className="mt-3 flex items-center gap-2 text-sm text-foreground/75"><span>{artist.guitar}</span>{artist.guitarUrl && <a href={artist.guitarUrl} target="_blank" rel="noreferrer" aria-label={t("ดูกีตาร์รุ่นนี้", "View this guitar")} className="text-brand hover:text-gold"><Guitar className="h-4 w-4" /></a>}</div>}</div></section>
