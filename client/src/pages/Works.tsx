...
function WorksRail({ items, emptyLabel, showAll }: { items: WorkItem[]; emptyLabel: string; showAll: boolean }) {
  ...
  return <div className={showAll ? "grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "flex snap-x gap-4 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"}>{items.map((item, index) => <div key={item.id} className={showAll ? "min-w-0" : "w-[88vw] max-w-[440px] shrink-0 snap-start sm:w-[48vw]"}><WorkCard item={item} index={index} square={showAll} /></div>)}</div>;
}
