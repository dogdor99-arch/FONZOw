...
export default function Works() {
  ...
  return <><div className="border-b border-border/70 bg-cream/40 px-4 py-4 sm:px-6 lg:px-10"><div className="mx-auto max-w-[1400px] lg:pl-12"><p className="text-[9px] tracking-[0.14em] text-muted-foreground uppercase">{t("หน้าแรก", "Home")} <span className="mx-1.5 text-brand">›</span> Works</p><h1 className="mt-2 font-display text-3xl leading-none sm:text-4xl">Works</h1></div></div><section className="mx-auto max-w-[1400px] px-4 pb-12 pt-3 sm:px-6 lg:px-10 lg:pb-16 lg:pt-5"><div className="grid items-stretch gap-8 md:grid-cols-[minmax(220px,0.62fr)_minmax(0,1.38fr)] lg:gap-12"><div className="relative min-h-[30rem] overflow-hidden bg-secondary md:min-h-0"><img src="/works-student-feature.jpg" alt={t("ผลงานนักเรียน", "Student work")} className="h-full w-full object-cover" /></div><div className="flex min-w-0 flex-col justify-center gap-5">{renderSection("students", students, Users, t("นักเรียนของคุณเบิร์ด", "Bird's students"), t("ยังไม่มีข้อมูลนักเรียนในระบบ — เพิ่มได้จาก Admin", "No student profiles yet — add them from Admin."))}{renderSection("events", events, CalendarDays, t("การออกบูทและกิจกรรมของแบรนด์", "Brand events"), t("ยังไม่มี Event ในระบบ — เพิ่มได้จาก Admin", "No events yet — add them from Admin."))}</div></div></section></>;
}
