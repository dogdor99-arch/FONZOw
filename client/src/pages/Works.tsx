...
import { CalendarDays, ChevronLeft, ChevronRight, ExternalLink, Images, Users } from "lucide-react";
  ...
  return <div>
    <div className="grid gap-5 md:grid-cols-3">
      {visible.map((item, index) => {
        ...
        return <Reveal key={item.id} delay={index * 60}><article className="group overflow-hidden border border-border/80 bg-card transition hover:-translate-y-1 hover:border-brand/50 hover:shadow-lg"><div className="aspect-[4/3] overflow-hidden bg-secondary">{image ? <img src={image} alt={title} className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" /> : <div className="flex h-full items-center justify-center text-muted-foreground"><Images className="h-8 w-8" strokeWidth={1.2} /></div>}</div><div className="p-5"><div className="flex flex-wrap items-center gap-3 text-[10px] tracking-[0.14em] text-brand uppercase">{item.eventDate && <span className="inline-flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" />{item.eventDate}</span>}<span>FONZO / {item.kind === "student" ? "STUDENT" : "EVENT"}</span></div><h3 className="mt-3 font-display text-2xl leading-tight">{title}</h3>{description && <p className="mt-3 line-clamp-4 text-sm leading-relaxed text-muted-foreground">{description}</p>}{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-[10px] font-semibold tracking-[0.14em] text-brand uppercase hover:underline">{t("ดูเรื่องราวต้นฉบับ", "View original story")}<ExternalLink className="h-3.5 w-3.5" /></a>}</div></article></Reveal>;
      })}
    </div>
