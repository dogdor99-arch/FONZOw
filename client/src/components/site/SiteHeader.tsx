    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white">
      <nav className="flex items-center justify-between px-4 py-3">
        ...
        <div className="hidden md:flex gap-x-6 text-sm font-medium">
          <Link href="/guitar" className="text-gray-600 hover:text-black">กีตาร์ทั้งหมด</Link>
      ...
      {isOpen && (
        <nav className="fixed inset-x-0 top-[57px] bottom-0 bg-white z-50 md:hidden overflow-y-auto border-t">
          <ul className="flex flex-col p-4 space-y-1 text-base font-medium">
            <li>
              <Link href="/guitar" onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-lg hover:bg-gray-100 border-b border-gray-100">
                กีตาร์ทั้งหมด
              </Link>
            </li>
            <li>
