...
export const worksRouter = router({
  uploadImage: adminProcedure.input(imageUploadInput).mutation(async ({ input }) => ({ url: await uploadImageToCloudinary(input.base64, input.contentType, "fonzo/works") })),
  list: publicProcedure.input(z.object({ kind: z.enum(["event", "student"]).optional() }).optional()).query(async ({ input }) => {
    const db = await conn();
    const rows = input?.kind ? await db.select().from(worksItems).where(eq(worksItems.kind, input.kind)).orderBy(asc(worksItems.sortOrder), asc(worksItems.id)).limit(1000) : await db.select().from(worksItems).orderBy(asc(worksItems.kind), asc(worksItems.sortOrder), asc(worksItems.id)).limit(1000);
    return rows.filter(row => row.published);
  }),
  listAll: adminProcedure.query(async () => { const db = await conn(); return db.select().from(worksItems).orderBy(asc(worksItems.kind), asc(worksItems.sortOrder), asc(worksItems.id)).limit(1000); }),
