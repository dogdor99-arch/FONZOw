...
export const artistsRouter = router({
  uploadImage: adminProcedure.input(imageUploadInput).mutation(async ({ input }) => { const buffer = Buffer.from(input.base64.replace(/^data:[^;]+;base64,/, ""), "base64"); if (buffer.byteLength > 8 * 1024 * 1024) throw new TRPCError({ code: "BAD_REQUEST", message: "Image must be smaller than 8MB" }); const ext = input.contentType.split("/")[1] === "jpeg" ? "jpg" : input.contentType.split("/")[1]; const { url } = await storagePut(`artists/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`, buffer, input.contentType); return { url }; }),
