...
export async function ensureEditorialTables() {
  ...
  await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS \`chatRooms\` (\`id\` int AUTO_INCREMENT NOT NULL, \`visitorToken\` varchar(96) NOT NULL, \`name\` varchar(160), \`email\` varchar(320), \`phone\` varchar(40), \`status\` enum('open','closed') NOT NULL DEFAULT 'open', \`lastMessageAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, CONSTRAINT \`chatRooms_id\` PRIMARY KEY(\`id\`), CONSTRAINT \`chatRooms_visitorToken_unique\` UNIQUE(\`visitorToken\`), INDEX \`chatRooms_lastMessageAt_idx\` (\`lastMessageAt\`))`));
  await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS \`chatMessages\` (\`id\` int AUTO_INCREMENT NOT NULL, \`roomId\` int NOT NULL, \`senderType\` enum('visitor','admin') NOT NULL, \`body\` text NOT NULL, \`readAt\` timestamp NULL, \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT \`chatMessages_id\` PRIMARY KEY(\`id\`), INDEX \`chatMessages_roomId_idx\` (\`roomId\`))`));
  for (const statement of [
    "ALTER TABLE `artistProfiles` ADD COLUMN `galleryUrls` JSON NULL",
